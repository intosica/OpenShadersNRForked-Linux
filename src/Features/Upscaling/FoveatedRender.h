...
struct FoveatedRender
{
	...
	struct Settings
	{
		...
		// Flat-only: run Feature 18 before DLSS at the dynamic render size instead
		// of after the engine tonemapper at display size. Cost scales with the
		// pixel count the model is created at, so this is the whole point — but it
		// also moves the pass into linear HDR and puts a generative result inside
		// DLSS's temporal history, so it stays opt-in.
		bool neuralRenderingPreUpscale = false;
		// Scene-referred exposure applied before the display-referred encode the
		// model is fed. The engine's own adaptation runs later (ISHDR) and isn't
		// readable here, so this is a manual stand-in.
		float neuralRenderingPreUpscaleExposure = 1.0f;
		// Which curve builds the display-referred copy the model is shown. The
		// transfer divides the curve back out, so this does not change fidelity -
		// it changes what the model DECIDES, by changing how contrasty the image it
		// sees is. 0 = GT7 (matches the Post Processing tonemapper), 1 = Reinhard.
		uint neuralRenderingPreUpscaleCurve = 0;
		// NGX PerfQualityValue handed to Feature 18 at creation. 0 follows the
		// current DLSS quality mode; 1-6 override it. Applies to both routes.
		uint neuralRenderingQualityContract = 0;
		// DLSSNR.Hint.Render.Preset at creation. 0 leaves it to the runtime.
