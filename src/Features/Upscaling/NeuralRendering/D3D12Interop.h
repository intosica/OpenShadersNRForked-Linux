...
namespace NeuralRendering
{
	...
	public:
		...
		/// `existingDevice`, when non-null, is adopted instead of creating one. Frame
		/// generation already stands up a D3D12 device for the presentation sidecar;
		/// adding a second one puts two D3D12 devices and two NGX initialisations on a
		/// single adapter, all sharing resources with the same D3D11 device.
		bool Initialize(IDXGIAdapter* adapter, ID3D11Device* device, ID3D11DeviceContext* context,
			ID3D12Device* existingDevice = nullptr);
		void Shutdown();
