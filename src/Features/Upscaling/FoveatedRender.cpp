...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(
	...
	neuralRenderingPreUpscaleExposure,
	neuralRenderingPreUpscaleCurve,
	neuralRenderingQualityContract,
...
void FoveatedRender::ClampSettings()
{
	...
	settings.neuralRenderingPreUpscaleExposure = std::clamp(settings.neuralRenderingPreUpscaleExposure, 0.1f, 4.0f);
	settings.neuralRenderingPreUpscaleCurve = std::min(settings.neuralRenderingPreUpscaleCurve, 1u);
	settings.neuralRenderingQualityContract = std::min(settings.neuralRenderingQualityContract, 6u);
	settings.neuralRenderingOutputPreset = std::min(settings.neuralRenderingOutputPreset, 3u);
	// Pre-upscale is a flat-only route: it rewrites kMAIN before DLSS, which the VR
	// foveated subrect path owns. Clear it here so a hand-edited config can't leave
	// a dead toggle reading as enabled.
	if (globals::game::isVR)
		settings.neuralRenderingPreUpscale = false;
...
void FoveatedRender::DrawSettings()
{
	...
	if (ImGui::CollapsingHeader(T(TKEY("neural_rendering_header"), "DLSS Neural Rendering"), ImGuiTreeNodeFlags_DefaultOpen)) {
		// Frame generation only blocks in VR now. On flat, the interop adopts the
		// presentation sidecar's D3D12 device instead of standing up a second one, so
		// the two coexist; the VR route still refuses it in ApplyFoveatedLdr.
		const bool frameGenerationBlocks = globals::game::isVR &&
		...
		const bool supportedRoute = globals::features::upscaling.GetUpscaleMethod() == Upscaling::UpscaleMethod::kDLSS &&
			!frameGenerationBlocks &&
			(!globals::game::isVR || (GetDlssMode() == DlssMode::kDefault &&
				globals::features::upscaling.perfMode.IsHookActive()));
		if (!supportedRoute) {
			if (frameGenerationBlocks)
				Util::Text::Warning("VR: disable Frame Generation and restart the game before enabling DLSS Neural Rendering.");
			else
		...
		if (settings.neuralRenderingEnabled) {
			...
			// Inert by construction. DLSSNR.UICorrection is a real runtime key, but it
			// acts on the DLSSNR.UI / DLSSNR.UIAlpha inputs, and neither route has a UI
			// layer to hand over: the post-tonemap route runs before UI composite and the
			// pre-upscale route runs before DLSS. Left visible but disabled so it is not
			// mistaken for a working control, or silently re-added later.
			ImGui::BeginDisabled();
			ImGui::Checkbox(T(TKEY("neural_rendering_ui_correction"), "UI Correction"), &settings.neuralRenderingUICorrection);
			ImGui::EndDisabled();
			ImGui::TextDisabled("%s", T(TKEY("neural_rendering_ui_correction_note"),
				"No UI layer exists at either evaluation point, so there is nothing to correct."));
			if (custom)
				settings.neuralRenderingPreset = 0;

			// Feature-creation inputs, applied to whichever route is active. Both are
			// consumed by CreateFeature, so a change rebuilds the feature on the next
			// evaluate - no restart needed, unlike some other integrations.
			static const char* qualityContracts[] = { "Follow Current DLSS Mode", "DLAA", "Quality",
				"Balanced", "Performance", "Ultra Performance", "Ultra Quality" };
			int qualityContract = static_cast<int>(settings.neuralRenderingQualityContract);
			if (ImGui::Combo(T(TKEY("neural_rendering_quality_contract"), "NR Quality Contract"),
					&qualityContract, qualityContracts, IM_ARRAYSIZE(qualityContracts)))
				settings.neuralRenderingQualityContract = static_cast<uint>(qualityContract);
			if (auto _tt = Util::HoverTooltipWrapper()) {
				ImGui::Text("%s", T(TKEY("neural_rendering_quality_contract_tooltip"), "The real NGX PerfQualityValue used when Feature 18 is created. It is a model\npolicy input, so it changes how hard the model works rather than the output\nresolution - the output stays whatever the route feeds it.\n\nFollow Current DLSS Mode is the safe default. Lowering it is reported to trade\nquality for framerate; measure NeuralRendering in the Performance Overlay to\nsee what it actually costs on your setup."));
			}

			static const char* outputPresets[] = { "Runtime Default", "Preset 1", "Preset 2", "Preset 3" };
			int outputPreset = static_cast<int>(settings.neuralRenderingOutputPreset);
			if (ImGui::Combo(T(TKEY("neural_rendering_output_preset"), "NR Output Preset"),
					&outputPreset, outputPresets, IM_ARRAYSIZE(outputPresets)))
				settings.neuralRenderingOutputPreset = static_cast<uint>(outputPreset);
			if (auto _tt = Util::HoverTooltipWrapper()) {
				ImGui::Text("%s", T(TKEY("neural_rendering_output_preset_tooltip"), "The installed runtime's private DLSSNR.Hint.Render.Preset, chosen at feature\ncreation. NVIDIA does not publish names or semantics for these, hence the bare\nnumbers. Runtime Default leaves the choice to the model.\n\nWorth trying if the look does not match the other route - this is the runtime\nown output-character control, separate from Style and the input curve."));
			}

			// Flat-only: evaluate before DLSS instead of after the engine tonemapper.
			if (!globals::game::isVR) {
				ImGui::Separator();
				if (ImGui::Checkbox(T(TKEY("neural_rendering_pre_upscale"), "Evaluate Before Upscaling (experimental)"),
						&settings.neuralRenderingPreUpscale)) {
					// The route swap changes the extents Feature 18 was created at and hands
					// it a different image; keep neither the feature nor its history.
					NeuralRendering::Reset();
				}
				if (auto _tt = Util::HoverTooltipWrapper()) {
					ImGui::Text("%s", T(TKEY("neural_rendering_pre_upscale_tooltip"),
						"Runs Neural Rendering at the render resolution, before DLSS, instead of at\ndisplay resolution after the engine tonemapper.\n\nThe pass costs roughly 3.5 ms per megapixel, so at DLSS Quality this is about\n2.25x less work - the saving grows with how much DLSS is upscaling.\n\nTrade-offs: the scene is linear HDR at this point, so the model is fed a\ntonemapped copy and its change is transferred back as a ratio (tune with\nPre-Upscale Exposure below). DLSS then accumulates a generated result into\nits temporal history, which may smear or shimmer on fine detail.\n\nFlat only. Compare against it off before keeping it."));
				}
				if (settings.neuralRenderingPreUpscale) {
					static const char* preUpscaleCurves[] = { "GT7 (match post-processing)", "Reinhard (flatter, stronger)" };
					int preUpscaleCurve = static_cast<int>(settings.neuralRenderingPreUpscaleCurve);
					if (ImGui::Combo(T(TKEY("neural_rendering_pre_upscale_curve"), "Model Input Curve"),
							&preUpscaleCurve, preUpscaleCurves, IM_ARRAYSIZE(preUpscaleCurves))) {
						settings.neuralRenderingPreUpscaleCurve = static_cast<uint>(preUpscaleCurve);
						// Extents are unchanged, so the feature itself can stay - only the
						// accumulated history is now built against a different input.
						// Avoids the full teardown stutter a route swap causes.
						NeuralRendering::Renderer::Instance().ResetHistory();
					}
					if (auto _tt = Util::HoverTooltipWrapper()) {
						ImGui::Text("%s", T(TKEY("neural_rendering_pre_upscale_curve_tooltip"),
							"Which curve builds the copy the model is shown. It does not affect fidelity -\nthe transfer divides it back out, and a pixel the model leaves alone is\nunchanged either way. It changes what the model DECIDES.\n\nGT7 matches the tonemapper your post-processing chain actually runs, so the\nmodel sees roughly the final contrast and corrects less.\n\nReinhard is flatter with lifted blacks. The model reads that as washed out and\npushes local tone harder, which compounds with the real tonemapper downstream -\na stronger, more contrasty look."));
					}
					ImGui::SliderFloat(T(TKEY("neural_rendering_pre_upscale_exposure"), "Pre-Upscale Exposure"),
						&settings.neuralRenderingPreUpscaleExposure, 0.1f, 4.0f, "%.2f");
					if (auto _tt = Util::HoverTooltipWrapper()) {
						ImGui::Text("%s", T(TKEY("neural_rendering_pre_upscale_exposure_tooltip"),
							"Exposure applied before the tonemapped copy handed to the model. The engine's\nown adaptation runs later and cannot be read here, so this is a manual stand-in.\nRaise it if the effect is weak in dark scenes, lower it if bright scenes look flat."));
					}
				}
			}
