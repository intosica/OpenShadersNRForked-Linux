...
namespace NeuralRendering
{
	...
	public:
			...
			std::uint32_t depthSourceWidth, std::uint32_t depthSourceHeight)
		{
			...
			// Creation-tuning changes need the same drain EnsureResources does before it
			// releases a feature. Must happen before BeginD3D12 opens a command list.
			if (Runtime::Instance().RequiresRebuild(eyeIndex, tuning)) {
				if (!interop.WaitForIdle())
					return LatchFailure("wait for idle before feature rebuild", interop.LastError());
			...
			if (!CopyDepthGuide(context, depthSRV, eye.depth.uav11.Get(), guideWidth, guideHeight,
					depthSourceWidth, depthSourceHeight))
				return LatchFailure("depth guide conversion", E_FAIL);
			// Subrect copy, not CopyResource: the guide textures are sized to the
			// *dynamic render size*, while the engine's motion vector target is a
			// display-res allocation whose valid region is the top-left subrect of
			// that size (EncodeTexturesCS dispatches over the render extent only).
			// CopyResource requires identical dimensions and would fail outright on
			// the pre-upscale route. Where the two already match — the post-tonemap
			// route at DLAA — the box covers the whole texture and this is a no-op
			// change.
			...
			std::uint32_t colorWidth, std::uint32_t colorHeight, const Tuning& tuning)
		{
			...
			for (std::uint32_t eyeIndex = 0; eyeIndex < inputs.size(); ++eyeIndex) {
				...
				if (!CopyDepthGuide(context, input.depthSRV, eye.depth.uav11.Get(), guideWidth, guideHeight,
						input.depthSourceWidth, input.depthSourceHeight))
					return LatchFailure("depth guide conversion", E_FAIL);
	...
	private:
			...
			std::uint32_t sourceWidth, std::uint32_t sourceHeight)
		{
			...
			if (!shader || !destination)
				return false;
			if (!depthGuideCB)
				depthGuideCB = std::make_unique<ConstantBuffer>(
					ConstantBufferDesc<DepthGuideData>(), "NeuralRendering::DepthGuideCB");
			// Zero means the caller's source is already guide-sized.
			DepthGuideData guideData{
				sourceWidth ? sourceWidth : width,
				sourceHeight ? sourceHeight : height,
				width,
				height
			};
			depthGuideCB->Update(guideData);

			context->CSSetShader(shader, nullptr, 0);
			context->CSSetShaderResources(0, 1, &source);
			context->CSSetUnorderedAccessViews(0, 1, &destination, nullptr);
			ID3D11Buffer* guideBuffer = depthGuideCB->CB();
			context->CSSetConstantBuffers(0, 1, &guideBuffer);
		...
		bool InitializeInterop(ID3D11Device* device, ID3D11DeviceContext* context)
		{
			...
			// Only present when frame generation stood the sidecar up; null otherwise, in
			// which case the interop creates its own device as before.
			auto& upscaling = globals::features::upscaling;
			ID3D12Device* sidecarDevice = upscaling.d3d12SwapChainActive ?
			                                  upscaling.dx12SwapChain.d3d12Device.get() :
			                                  nullptr;
			bool interopReady = SUCCEEDED(result) &&
			                    interop.Initialize(adapter.Get(), device, context, sidecarDevice);

			// The sidecar's device is Streamline-proxied when DLSS-G is active
			// (Upscaling.cpp runs slUpgradeInterface on it). A shared fence created
			// through that proxy produces a handle the D3D11 device cannot open as an
			// ID3D11Fence, so OpenSharedFence fails E_NOINTERFACE. The fence and the
			// queue that signals it have to share a device, so there is no way to mix
			// the two - fall back to a private device wholesale rather than losing NR.
			if (!interopReady && SUCCEEDED(result) && sidecarDevice) {
				logger::warn("[DLSSNR] sidecar device rejected at step '{}' (hr=0x{:08X}); "
							 "retrying with a private D3D12 device",
					interop.LastOperation(), static_cast<std::uint32_t>(interop.LastError()));
				// Initialize() begins with Shutdown(), so the retry starts clean.
				interopReady = interop.Initialize(adapter.Get(), device, context, nullptr);
			}

			if (!interopReady) {
				logger::error("[DLSSNR] interop init failed at step '{}' sidecar={} hr=0x{:08X}",
					interop.LastOperation(), sidecarDevice ? "offered" : "none",
					static_cast<std::uint32_t>(FAILED(result) ? result : interop.LastError()));
		...
		std::unique_ptr<ConstantBuffer> depthGuideCB;
