...
namespace NeuralRendering
{
		...
		float motionVectorScaleX, float motionVectorScaleY, const Tuning& tuning, bool reset)
	{
		...
		// Only dimensions are handled here. Creation-tuning changes are detected by the
		// caller BEFORE it opens a command list, because releasing a feature that
		// in-flight command lists still reference hangs the device - the fence never
		// signals, the command contexts saturate, and BeginD3D12 times out.
		...
		if (!featureHandles_[slot]) {
			// Only keys the 310.8 runtime actually reads. A string scan of
			// nvngx_dlssnr.dll showed ten of the names previously set here are absent
			// from the binary - NGX parameter maps accept any key, so they were silently
			// ignored rather than failing. Notably DLSSNR.Upscaling and DLSSNR.Scale do
			// not exist: the feature has no colour-rescaling mode, and its cost is purely
			// a function of the extents it is created at.
			parameters->Reset();
			...
			featureQuality_[slot] = tuning.performanceQuality;
			featurePreset_[slot] = tuning.outputPreset;
			reset = true;
		}
	...
	bool Runtime::RequiresRebuild(std::uint32_t slot, const Tuning& tuning) const
	{
		if (slot >= 2 || !featureHandles_[slot])
			return false;
		return featureQuality_[slot] != tuning.performanceQuality ||
		       featurePreset_[slot] != tuning.outputPreset;
	}
	...
	void Runtime::ResetFeature(std::uint32_t slot)
	{
		...
		featureQuality_[slot] = featurePreset_[slot] = UINT32_MAX;
	}
