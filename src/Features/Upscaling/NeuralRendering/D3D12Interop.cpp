...
namespace NeuralRendering
{
	...
	bool D3D12Interop::Initialize(IDXGIAdapter* adapter, ID3D11Device* device, ID3D11DeviceContext* context,
		ID3D12Device* existingDevice)
	{
		Shutdown();
		if (!adapter || !device || !context)
			return RecordFailure(E_INVALIDARG);

		lastOperation_ = "QueryInterface(ID3D11Device5)";
		HRESULT result = device->QueryInterface(IID_PPV_ARGS(&device11_));
		if (FAILED(result)) return RecordFailure(result);
		lastOperation_ = "QueryInterface(ID3D11DeviceContext4)";
		result = context->QueryInterface(IID_PPV_ARGS(&context11_));
		if (FAILED(result)) return RecordFailure(result);
		// Adopt the presentation sidecar's device when one exists rather than standing up
		// a second. The queue and command allocators below stay ours regardless: this pass
		// records mid-frame, and sharing the sidecar's queue would interleave our work with
		// presentation in an order neither side expects.
		if (existingDevice) {
			lastOperation_ = "adopt sidecar ID3D12Device";
			device12_ = existingDevice;  // WRL ComPtr assignment AddRefs
			ownsDevice_ = false;
		} else {
			lastOperation_ = "D3D12CreateDevice";
		...
		// NOTE: this interop is broken by MSVC link-time code generation. Under
		// /GL + /LTCG every REFIID-taking call here fails E_NOINTERFACE - originally
		// OpenSharedFence, and CreateFence too once the sharing direction was flipped -
		// while /Od and plain /O2 both work. Reversing the direction of the fence share
		// was tried and did not help, so the direction below is the original one.
		//
		// Unresolved. A likely contributor is an ODR violation elsewhere in the project:
		// DX12SwapChain.h, Upscaling.cpp, FidelityFX.cpp and RuntimeUpscaler.cpp include
		// <directx/d3dx12.h> (vcpkg DirectX-Headers) while this file includes <d3d12.h>
		// (Windows SDK). Both use the __D3D12_H__ guard, so whichever is reached first
		// wins per translation unit and the D3D12 interfaces end up declared differently
		// across the binary - exactly the kind of thing LTCG stops tolerating.
		//
		// Build with CMAKE_INTERPROCEDURAL_OPTIMIZATION=OFF until this is understood.
		lastOperation_ = "CreateFence(SHARED)";
		result = device12_->CreateFence(0, D3D12_FENCE_FLAG_SHARED, IID_PPV_ARGS(&fence12_));
		if (FAILED(result)) return RecordFailure(result);
		HANDLE sharedFence = nullptr;
		lastOperation_ = "CreateSharedHandle(fence)";
		result = device12_->CreateSharedHandle(fence12_.Get(), nullptr, GENERIC_ALL, nullptr, &sharedFence);
		if (FAILED(result)) return RecordFailure(result);
		lastOperation_ = "OpenSharedFence";
